...
#define BWT 6000
#define BYTES_PER_SLOT 100  //Size of each packet
#define UGS_BUFFER_LIMIT 100000
#define RTPS_BUFFER_LIMIT 100000
#define NRTPS_BUFFER_LIMIT 100000
#define ERTPS_BUFFER_LIMIT 100000
#define BE_BUFFER_LIMIT 100000
...
char option;
bool fileEntryy;
double allocated_Bandwidth, ugsBandwidth_Allocated, rtpsBandwidth_Allocated, nrtpsBandwidth_Allocated, ertpsBandwidth_Allocated, beBandwidth_Allocated;
Network n1;
...
int convertDataRatetoSlots(double dataRate, float sinr);
void initializeBandwidth_Allocated();
void Bandwidth_Allocated(int type, double bandwidth);
int sizeAsPerQAM(float snr);
...
int main(){
	...
	if(option=='y' || option=='Y'){
		fileEntryy = true;
		n1.readFile();	 //And store the request information in data vector
...
/* Write to a file */
	...
	displayResult();
	displayReqData();
        std::cout<<"total slots allocated: "<<2048-TOTAL_SLOTS<<"\n";
	if(fileEntryy){
		writeReq();
	}
	/*
		...
		for(int i = 1; i<= NETWORK_NODES; i++){                  // i is the Station number
			...
			if(i==1){
				...
				if(!fileEntryy){
					...
					for(int k = 0; k < req; ++k)
    					   station1request[k] = new double[9]; 
				}
			}
			if(i==2){
				 row2= req;
				if(!fileEntryy){
					...
					for(int k = 0; k < req; ++k)
	    				   station2request[k] = new  double[9];
				}
			}
			if(i==3){
				row3= req;
				if(!fileEntryy){
					...
					for(int k = 0; k < req; ++k)
    				   		station3request[k] = new  double[9];
				}
			}
			if(i==4){
				row4= req;
			...
			row5= req;
				if(!fileEntryy){
					...
					for(int k = 0; k < req; ++k)
    					   station5request[k] = new  double[9];
				}
			}
			if(i==6){
			row6= req;
				if(!fileEntryy){
					...
					for(int k = 0; k < req; ++k)
    				   		station6request[k] = new  double[9];
				}
			}
				...
				while((diff<1) && (req>0) ){
					...
					//network.insert(std::pair<int,Request> (i,r1));             //Storing of request in network
				 	if(i == 1){
						...
						if(!fileEntryy){
							...
							station1request[iterator][8] = r1.data_rate;
						}
					...
					if(i == 2){
						reqsS2.push(r1);
							...
							station2request[iterator][8] = r1.data_rate;
							}
					...
					if(i == 3){
						reqsS3.push(r1);
							...
							station3request[iterator][8] = r1.data_rate;
							}
					...
					if(i == 4){
						reqsS4.push(r1);
							...
							station4request[iterator][8] = r1.data_rate;
							}
					...
					if(i == 5){
						...
						if(!fileEntryy){
							...
							station5request[iterator][8] = r1.data_rate;
					...
					if(i == 6){
						...
						if(!fileEntryy){
							...
							station6request[iterator][8] = r1.data_rate;
						}
...
void allocateSlots(double dataRate, float sinr){
	int slotsAllocated = 0;
	slotsAllocated = convertDataRatetoSlots(dataRate,sinr);
...
void freeBandwidth(int type,int bandwidth,Request req_obj){
	...
	if(req_obj.flag != -1){
		...
		allocateSlots(req_obj.data_rate,req_obj.sinr);
	}
...
int convertDataRatetoSlots(double dataRate, float sinr){
	...
	int slots;
	/*slots = dataRate/BYTES_PER_SLOT;
	if(dataRate < BYTES_PER_SLOT){
	 slots = 1;	
	}*/
	//std::cout<<"Data Rate test "<<dataRate<<"\n";
	int no_of_packets = (int)(dataRate*1000)/100;
	//std::cout<<"no of packets"<<no_of_packets<<" "<<sizeAsPerQAM(sinr)<<sinr<<"\n";
	int no_of_slots = 100/sizeAsPerQAM(sinr);
	//std::cout<<"no of slot"<<no_of_slots<<"\n";
	slots = no_of_packets * no_of_slots;
	//std::cout<<"sllot"<<slots<<"\n";
	return slots;
}


int sizeAsPerQAM(float snr){
	//std::cout<<"What is snr? test"<<snr<<"\n";
	//snr = ()snr;
	if(snr < 7){  //6.4
		return 4;
	}
	if(snr > 7 && snr < 12){  //11.2
		return 9;
	}
	if(snr > 12 && snr < 19){ //18.2
		return 18;
	}
	if(snr> 20){ //24.4
		return 30;
...
int allocateBandwidthUGS(int bandwidth_available,int check){
	...
	while(!ugs.empty()){
		...
		if(isBandwidthAvailable(r.bandwidth,bandwidth_available)){
			bandwidth_available -=r.bandwidth;
			Bandwidth_Allocated(10,r.bandwidth);
			reqSatisfy.push(r);
			if(b>0){
				r.bandwidth = b;
				temp.push(r);			
			...
			Bandwidth_Allocated(10,bandwidth_available);
			reqSatisfy.push(r1);
			if(r.flag == 0){
			r.flag = -1;
			}
...
int allocateBandwidthRTPS(int bandwidth_available,int check){
	...
	while(!rtps.empty()){
		...
		if(isBandwidthAvailable(r.bandwidth,bandwidth_available)){
			bandwidth_available -=r.bandwidth;
			Bandwidth_Allocated(9,r.bandwidth);
			reqSatisfy.push(r);
			if(b>0){
				r.bandwidth = b;
				temp.push(r);			
			...
			Bandwidth_Allocated(9,bandwidth_available);
			reqSatisfy.push(r1);
			if(r.flag ==0){
			r.flag = -1;
			}
...
int allocateBandwidthNRTPS(int bandwidth_available,int check){
	...
	while(!nrtps.empty()){
		...
		if(isBandwidthAvailable(r.bandwidth,bandwidth_available)){
			bandwidth_available -=r.bandwidth;
			Bandwidth_Allocated(8,r.bandwidth);
			reqSatisfy.push(r);
			if(b>0){
				r.bandwidth = b;
				temp.push(r);			
			...
			Bandwidth_Allocated(8,bandwidth_available);
			reqSatisfy.push(r1);
			if(r.flag == 0){
			r.flag = -1;
			}
...
int allocateBandwidthERTPS(int bandwidth_available,int check){
	...
	while(!ertps.empty()){
		...
		if(isBandwidthAvailable(r.bandwidth,bandwidth_available)){
			bandwidth_available -=r.bandwidth;
			Bandwidth_Allocated(7,r.bandwidth);
			reqSatisfy.push(r);
			if(b>0){
				r.bandwidth = b;
				temp.push(r);			
			...
			Bandwidth_Allocated(7,bandwidth_available);
			reqSatisfy.push(r1);
			if(r.flag == 0){
			r.flag = -1;
			}
...
int allocateBandwidthBE(int bandwidth_available,int check){
	...
	while(!be.empty()){
		...
		if(isBandwidthAvailable(r.bandwidth,bandwidth_available)){
			bandwidth_available -=r.bandwidth;
			Bandwidth_Allocated(6,r.bandwidth);
			reqSatisfy.push(r);
			if(b>0){
				r.bandwidth = b;
				temp.push(r);			
			...
			Bandwidth_Allocated(6,bandwidth_available);
			reqSatisfy.push(r1);
			if(r.flag == 0){
			r.flag = -1;
			}
...
void createRequestFile(){
	std::ofstream myfile;
	myfile.open ("./result/station/request1.txt");
	...
	myfile.open ("./result/station/request2.txt");
	//int rowCount,colCount;
	//cout<<"Enter row and column numbers\n";
	//cin>>rowCount>>colCount;
			rowCount = row2;
	...
	myfile.close();	
	myfile.open ("./result/station/request3.txt");
	//int rowCount,colCount;
	//cout<<"Enter row and column numbers\n";
	//cin>>rowCount>>colCount;
			rowCount = row3;
	...
	myfile.close();	
	myfile.open ("./result/station/request4.txt");
	//int rowCount,colCount;
	//cout<<"Enter row and column numbers\n";
	//cin>>rowCount>>colCount;
			rowCount = row4;
	...
	myfile.close();	
	myfile.open ("./result/station/request5.txt");
	//int rowCount,colCount;
	//cout<<"Enter row and column numbers\n";
	//cin>>rowCount>>colCount;
			rowCount = row5;
	...
	myfile.close();	
	myfile.open ("./result/station/request6.txt");
	//int rowCount,colCount;
	//cout<<"Enter row and column numbers\n";
	//cin>>rowCount>>colCount;
			rowCount = row6;
...
//UGS
  	outfile.open("./result/traffic/denormal/ugs/ugsresult.txt",std::ofstream::out | std::ofstream::app);
	...
	outfile.close();
//RTPS
	outfile.open("./result/traffic/denormal/rtps/rtpsresult.txt",std::ofstream::out | std::ofstream::app);
	...
	outfile.close();
//NRTPS
	outfile.open("./result/traffic/denormal/nrtps/nrtpsresult.txt",std::ofstream::out | std::ofstream::app);
	...
	outfile.close();
//ERTPS
	outfile.open("./result/traffic/denormal/ertps/ertpsresult.txt",std::ofstream::out | std::ofstream::app);
	...
	outfile.close();
//BE
	outfile.open("./result/traffic/denormal/be/beresult.txt",std::ofstream::out | std::ofstream::app);
...
void saveResultNormalised(){
	...
	std::ofstream outfile;
//UGS
  	outfile.open("./result/traffic/normal/ugs/ugsresult1.txt",std::ofstream::out | std::ofstream::app);
	...
	outfile.close();
//RTPS
	outfile.open("./result/traffic/normal/rtps/rtpsresult1.txt",std::ofstream::out | std::ofstream::app);
	temp = rtps_served-rtps_discarded+rtps.size();
	temp1 = n1.rtps_generated-rtps_discarded;
	temp = std::min(temp,temp1);
	//rtps_result = n1.rtps_generated+" "+temp;
	...
	outfile.close();
//NRTPS
	outfile.open("./result/traffic/normal/nrtps/nrtpsresult1.txt",std::ofstream::out | std::ofstream::app);
	temp = nrtps_served-nrtps_discarded+nrtps.size();
	temp1 = n1.nrtps_generated-nrtps_discarded;
	temp = std::min(temp,temp1);
	//nrtps_result = n1.nrtps_generated+" "+temp;
	...
	outfile.close();
//ERTPS
	outfile.open("./result/traffic/normal/ertps/ertpsresult1.txt",std::ofstream::out | std::ofstream::app);
	temp = ertps_served-ertps_discarded+ertps.size();
	temp1= n1.ertps_generated-ertps_discarded;
	temp = std::min(temp,temp1);
	outfile << n1.ertps_generated <<"|" <<temp1<<std::endl;
	...
	outfile.close();
//BE
	outfile.open("./result/traffic/normal/be/beresult1.txt",std::ofstream::out | std::ofstream::app);
	temp = be_served-be_discarded+be.size();
	temp1 = n1.be_generated-be_discarded;
	temp = std::min(temp,temp1);
	outfile << n1.be_generated <<"|" <<temp1<<std::endl;
...
void displayResult(){
	...
	std::cout<<"ugs Bandwidth: "<<ugsBandwidth_Allocated<<"/"<<n1.ugs_bandwidth<<"\n";
	std::cout<<"rtps Bandwidth: "<<rtpsBandwidth_Allocated<<"/"<<n1.rtps_bandwidth<<"\n";
	std::cout<<"nrtps Bandwidth: "<<nrtpsBandwidth_Allocated<<"/"<<n1.nrtps_bandwidth<<"\n";
	std::cout<<"ertps Bandwidth: "<<ertpsBandwidth_Allocated<<"/"<<n1.ertps_bandwidth<<"\n";
	std::cout<<"be Bandwidth: "<<beBandwidth_Allocated<<"/"<<n1.be_bandwidth<<"\n";
	std::cout<<"Slots allocated: "<<2048-TOTAL_SLOTS<<"\n";
}
...
void initializeBandwidth_Allocated(){
	...
	ugsBandwidth_Allocated = 0; 
	rtpsBandwidth_Allocated =0; 
	nrtpsBandwidth_Allocated =0;
        ertpsBandwidth_Allocated = 0;
 	beBandwidth_Allocated = 0;
}

void Bandwidth_Allocated(int type, double bandwidth){

	switch(type){
		case 10:ugsBandwidth_Allocated+=bandwidth;
			break;
		case 9: rtpsBandwidth_Allocated += bandwidth;
			break;
		case 8: nrtpsBandwidth_Allocated += bandwidth;
			break;
		case 7: ertpsBandwidth_Allocated += bandwidth;
			break;		
		case 6: beBandwidth_Allocated += bandwidth;
		}
}
