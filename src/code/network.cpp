...
/*	  for (int y = 0; y < data1.size(); y++)
	...
	//std::cout<<bw<<"This is the bandwidth allocated\n";
		...
		std::cout<<"test5\n";
		fl = 0;
		dr = allocateValue(source,iterator,8);
		calculateTrafficClass(typ);
		//std::cout<<"TYPE is: "<<typ<<"\n";
	}
	//std::cout<<"test2\n";
	bandwidthRequired(typ,bw);
...
void Network::readFile(){
    ...
    std::ifstream file("./result/station/request1.txt");// file path
    std::string line;
...
*/
	file.close();
	file.open("./result/station/request2.txt");
...
std::cout<<"Data2:\n";
	...
	file.open("./result/station/request3.txt");
...
std::cout<<"Data3:\n";
	...
	file.open("./result/station/request4.txt");
...
std::cout<<"Data4:\n";
	...
	file.open("./result/station/request5.txt");
...
std::cout<<"Data5:\n";
	...
	file.open("./result/station/request6.txt");
...
double Network::assignDataRate(float snr, int typ1){
	if(typ1 == 10){
		return 1;	
	}
	if(typ1 == 9){
...
void Network::initializeBandwidth(){
	ugs_bandwidth = 0;
	nrtps_bandwidth = 0;
	rtps_bandwidth = 0;
	ertps_bandwidth = 0;
...
void Network::bandwidthRequired(int typ1,double bw1){
	if(typ1 == 10){
		ugs_bandwidth +=bw1;	
	}
	if(typ1 == 9){
